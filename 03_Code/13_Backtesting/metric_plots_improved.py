...
Creates bar charts, scatter plots, and comparison tables with enhanced features
"""
...
def plot_risk_return_scatter(metrics_dict: Dict[str, Dict],
    ...
    if add_frontier and len(risks) > 2:
        ...
        if len(efficient_points) > 1:
            ...
            ax.fill_between(eff_risks, min(returns) - 5, eff_returns,
...
def create_all_metric_plots(metrics_dict: Dict[str, Dict],
                           nav_dict: Dict[str, pd.Series] = None,
                           title_suffix: str = "",
                           base_path: str = './plots/metrics/') -> Dict[str, plt.Figure]:
    """
    ...
    # Create all plots
    figures['sharpe_bars'] = plot_sharpe_comparison(
        metrics_dict, title_suffix,
        save_path=os.path.join(base_path, 'sharpe_bars.png')
    )
    
    figures['total_returns_bars'] = plot_returns_comparison(
        metrics_dict, ['total_return'], title_suffix,
        save_path=os.path.join(base_path, 'total_returns_bars.png')
    )
    
    figures['cagr_bars'] = plot_returns_comparison(
        metrics_dict, ['cagr'], title_suffix,
        save_path=os.path.join(base_path, 'cagr_bars.png')
    )
    
    figures['risk_return_scatter'] = plot_risk_return_scatter(
        metrics_dict, title_suffix,
        save_path=os.path.join(base_path, 'risk_return_scatter.png')
    )
    
    figures['drawdown_bars'] = plot_drawdown_comparison(
        metrics_dict, title_suffix,
        save_path=os.path.join(base_path, 'drawdown_bars.png')
    )
    
    figures['win_rate_comparison'] = plot_win_rate_comparison(
        metrics_dict, nav_dict, title_suffix,
        save_path=os.path.join(base_path, 'win_rate_comparison.png')
    )
    
    figures['metrics_table'] = plot_metrics_table(
        metrics_dict, title_suffix=title_suffix,
        save_path=os.path.join(base_path, 'metrics_table.png')
    )
    
    print(f"✓ Created all metric plots in {base_path}")
