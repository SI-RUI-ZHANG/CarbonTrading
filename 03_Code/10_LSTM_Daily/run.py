"""
Complete LSTM Training Pipeline for Daily Carbon Price Direction Prediction
...
def prepare_data(config, save_to_disk=False):
    ...
    input_path = f'../../02_Data_Processed/03_Feature_Engineered/{config.MARKET}_LSTM_advanced.parquet'
...
def train_model(config):
    ...
    np.save(f'{config.OUTPUT_DIR}/test_probabilities.npy', probabilities)
    
    logger.info(f"All results saved to: {config.OUTPUT_DIR}")
    
    # Update performance tracker
    update_performance_summary(
        model_type='daily_base',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train LSTM Daily Model')
    ...
    logger.info("\n" + "="*80)
    logger.info("LSTM DAILY CARBON PRICE DIRECTION PREDICTION")
